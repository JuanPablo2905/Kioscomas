...
export function PromptDialog({ open, title, message, value, onChange, placeholder = "", confirmLabel = "Confirmar", cancelLabel = "Cancelar", readOnly = false, busy = false, error = "", onConfirm, onCancel }) {
  if (!open) return null;
  return createPortal(<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/55 p-4 backdrop-blur-[2px]"><div role="dialog" aria-modal="true" className="app-confirm w-full max-w-md overflow-hidden rounded-2xl border shadow-2xl"><div className="flex items-start justify-between gap-3 border-b p-5"><div><h2 className="font-bold">{title}</h2>{message&&<p className="mt-1 text-sm opacity-70">{message}</p>}</div><button type="button" disabled={busy} onClick={onCancel} aria-label="Cerrar" className="rounded-lg p-1.5 hover:bg-black/5 disabled:opacity-40"><X size={18}/></button></div><div className="p-4"><textarea autoFocus={!readOnly} readOnly={readOnly || busy} value={value} onChange={(event)=>onChange?.(event.target.value)} placeholder={placeholder} rows={readOnly?8:4} className="w-full resize-y rounded-xl border bg-transparent px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[var(--app-primary,#1d564d)] disabled:opacity-60"/>{error&&<p role="alert" className="mt-3 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold leading-5 text-red-700">{error}</p>}<div className="mt-4 grid grid-cols-2 gap-2"><button type="button" disabled={busy} onClick={onCancel} className="rounded-xl border px-4 py-2.5 text-sm font-semibold disabled:opacity-40">{cancelLabel}</button><button type="button" disabled={busy} onClick={onConfirm} className="rounded-xl bg-[var(--app-primary,#1d564d)] px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-55">{busy ? "Procesando…" : confirmLabel}</button></div></div></div></div>, document.body);
}
