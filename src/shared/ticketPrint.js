...
export function printTicket(ticket, { businessName = "Mi negocio", paper = "80", template = {}, reprint = false } = {}) {
  ...
  const visibleNumber = `${String(presentation.prefijo || "")}${ticket.numero || ticket.id}`;
  ...
  const ticketLine = Array.from(popup.document.querySelectorAll("p")).find((node) => node.textContent.startsWith("Ticket #"));
  if (presentation.numeroVisible === false) {
    ticketLine?.remove();
    popup.document.title = "Comprobante de venta";
  } else if (ticketLine) {
    ticketLine.textContent = `Ticket #${visibleNumber}`;
    popup.document.title = `Ticket ${visibleNumber}`;
  }
