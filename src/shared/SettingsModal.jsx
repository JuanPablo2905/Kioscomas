...
import { Archive, Cloud, Copy, Download, Gift, ImagePlus, LayoutPanelLeft, Palette, Printer, RefreshCw, Settings, ShieldCheck, SlidersHorizontal, Store, Trash2, X } from "lucide-react";
...
export function SettingsModal({ preferences, onChange, onClose, cuenta, tenantId, onUpdateAccount, canEditBusiness = true, onCleanOperationalHistory, onExportCommercialArchive, archiveStats, syncStatus }) {
  ...
  return <div className={`settings-overlay fixed inset-0 z-50 flex items-stretch justify-center bg-black/45 p-0 sm:items-center sm:p-4 ${closing ? "is-closing" : ""}`} onMouseDown={(event) => event.target === event.currentTarget && close()}>
    <div className="settings-panel flex h-[100dvh] w-full min-w-0 max-w-6xl flex-col overflow-hidden border bg-white shadow-2xl sm:h-[min(780px,94vh)] sm:flex-row sm:rounded-2xl">
      <aside className="settings-navigation w-full shrink-0 border-b bg-gray-50 sm:w-60 sm:border-b-0 sm:border-r sm:p-4">
        ...
        <div className="settings-section-tabs flex min-w-max gap-1 sm:block sm:min-w-0 sm:space-y-1">{sections.map(([id, label, Icon]) => <button key={id} onClick={() => setSection(id)} className={`flex shrink-0 items-center gap-2 whitespace-nowrap rounded-lg px-3 py-2 text-left text-sm sm:w-full ${section === id ? "bg-gray-900 text-white" : "text-gray-600 hover:bg-gray-100"}`}><Icon className="shrink-0" size={16}/>{label}</button>)}</div>
      </aside>
      <main className="min-w-0 flex-1 overflow-x-hidden overflow-y-auto p-4 sm:p-7"><div className="mb-5 flex items-start justify-between gap-3 sm:mb-6"><div className="min-w-0"><h2 className="break-words text-xl font-bold">{sections.find(([id]) => id === section)?.[1]}</h2><p className="break-words text-sm text-gray-500">Los cambios se guardan automáticamente para este usuario.</p><p className="mt-1 break-words text-xs font-semibold text-gray-500">Kiosco+ v{displayedVersion} · {appModeLabel}</p></div><button onClick={close} className="shrink-0 rounded-lg p-2"><X size={20}/></button></div>
