...
export function SettingsModal({ preferences, onChange, customerDisplayPreferences = {}, onCustomerDisplayChange, promotions = [], products = [], onClose, cuenta, tenantId, onUpdateAccount, canEditBusiness = true, onCleanOperationalHistory, onExportCommercialArchive, archiveStats, syncStatus, initialSection = "apariencia", notificationPreviewMode = false, onReportProblem }) {
    ...
    : desktopUpdate?.status === "available"
      ...
      : desktopUpdate?.status === "downloading"
        ...
        : desktopUpdate?.status === "downloaded"
          ? `Versión ${desktopUpdate.availableVersion} lista. Se instalará al cerrar Kiosco+.`
  ...
  return <div className={`settings-overlay fixed inset-0 z-50 flex items-stretch justify-center bg-black/45 p-0 sm:items-center sm:p-4 ${closing ? "is-closing" : ""}`} onMouseDown={(event) => event.target === event.currentTarget && close()}>
    ...
    {section === "nube" && <div className="grid min-w-0 gap-3">
      ...
      <div className={`min-w-0 rounded-xl border p-3 sm:p-4 ${desktopUpdate?.status === "downloaded" ? "border-green-200 bg-green-50" : desktopUpdate?.status === "error" ? "border-amber-200 bg-amber-50" : "bg-gray-50"}`}><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><b className="text-sm">Versión de Kiosco+</b><p className="mt-1 break-words text-xs text-gray-600">Versión actual: {displayedVersion}</p><p className="mt-1 break-words text-xs text-gray-600">{appModeLabel}</p>{window.kioscoDesktop?.updates && <p className="mt-1 break-words text-xs text-gray-600">{updateStatusText}</p>}{!window.kioscoDesktop?.updates && <p className="mt-1 break-words text-xs text-gray-600">La versión web se actualiza automáticamente al volver a abrirla.</p>}</div>{window.kioscoDesktop?.updates && <div className="flex shrink-0 flex-col gap-2 sm:flex-row">{desktopUpdate?.status === "downloaded" && <button type="button" onClick={() => window.kioscoDesktop.updates.install()} className="min-h-10 rounded-lg bg-green-700 px-3 py-2 text-xs font-semibold text-white">Reiniciar y actualizar</button>}{desktopUpdate?.supported !== false && <button type="button" disabled={["checking", "downloading"].includes(desktopUpdate?.status)} onClick={() => window.kioscoDesktop.updates.check()} className="flex min-h-10 items-center justify-center gap-2 rounded-lg border bg-white px-3 py-2 text-xs font-semibold disabled:opacity-50"><RefreshCw size={14} className={desktopUpdate?.status === "checking" ? "animate-spin" : ""}/>Buscar ahora</button>}</div>}</div></div>
