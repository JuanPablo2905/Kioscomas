...
export function SettingsModal({ preferences, onChange, onClose, cuenta, tenantId, onUpdateAccount, canEditBusiness = true, onCleanOperationalHistory, onExportCommercialArchive, archiveStats }) {
  ...
  return <div className={`settings-overlay fixed inset-0 z-50 flex items-stretch justify-center bg-black/45 p-0 sm:items-center sm:p-4 ${closing ? "is-closing" : ""}`} onMouseDown={(event) => event.target === event.currentTarget && close()}>
    ...
    {section === "nube" && <div className="grid min-w-0 gap-3">
      ...
      <Field label="Canal de actualizaciones"><Select value={cloudConfig.updateChannel} onChange={(v)=>saveCloud({updateChannel:v})}><option value="stable">Estable</option><option value="beta">Pruebas</option></Select></Field>
      <Field label="Comprobar actualizaciones automáticamente"><Toggle value={cloudConfig.autoCheckUpdates} onChange={(v)=>saveCloud({autoCheckUpdates:v})}/></Field>
