...
import { Archive, Bell, Cloud, Copy, Download, Gift, HelpCircle, ImagePlus, LayoutPanelLeft, Palette, Printer, RefreshCw, Settings, ShieldCheck, SlidersHorizontal, Store, Trash2, X } from "lucide-react";
import { isLocalCloudApiUrl, loadCloudConfig, saveCloudConfig } from "../cloud/config";
import { bootstrapCloud, cloudSession, loginCloud, logoutCloud, pairCloudDevice } from "../cloud/cloudAuth";
import { AppSelect, NumberInput } from "./controls";
import { BETA_TRIAL_DAYS, DEFAULT_MONTHLY_PLAN_PRICE, INTRODUCTORY_MONTHLY_PLAN_PRICE, INTRODUCTORY_PAID_MONTHS, paidBillingCycles } from "../billing/referrals";
...
import { ReleaseNotesHistory } from "../updates/ReleaseNotes";
import { DataRecoveryPanel } from "../features/configuracion/DataRecoveryPanel";
...
export const DEFAULT_PREFERENCES = {
  ...
  customerDisplayEnabled: false, customerDisplayAutoOpen: false, customerDisplayFullscreen: true, customerDisplayShowLogo: true,
  ...
  customerDisplayMercadoPagoMode: "ask", customerDisplayMercadoPagoTarget: "ask", customerDisplayMercadoPagoPosId: "", customerDisplayMercadoPagoTerminalId: "",
  tutorialsCompleted: [],
};
...
function DesktopUpdateCard({ desktopUpdate, displayedVersion, appModeLabel, updateStatusText }) {
  const updates = typeof window !== "undefined" ? window.kioscoDesktop?.updates : null;
  const busy = ["checking", "available", "downloading", "installing"].includes(desktopUpdate?.status);
  const ready = desktopUpdate?.status === "downloaded";
  const canManage = Boolean(updates) && desktopUpdate?.supported !== false;
  const cardTone = ready
    ? "border-green-200 bg-green-50"
    : desktopUpdate?.status === "error"
      ? "border-amber-200 bg-amber-50"
      : "bg-gray-50";
  const progress = Math.max(0, Math.min(100, Number(desktopUpdate?.percent || 0)));
  const availableNotes = String(desktopUpdate?.releaseNotes || "")
    .replace(/^#{1,6}\s*/gm, "")
    .replace(/^[-*]\s+/gm, "• ")
    .trim();
  return <div className={`min-w-0 rounded-2xl border p-4 sm:p-5 ${cardTone}`}>
    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
      <div className="min-w-0">
        <p className="text-xs font-black uppercase tracking-wide text-[#1C4A44]">Kiosco+ v{displayedVersion}</p>
        <h3 className="mt-1 text-lg font-bold">{appModeLabel}</h3>
        <p className="mt-1 text-xs font-semibold text-gray-500">Versión actual: {displayedVersion}</p>
        <p className="mt-2 break-words text-sm leading-6 text-gray-600">{updates ? updateStatusText : "La versión web se actualiza automáticamente al volver a abrirla."}</p>
        {desktopUpdate?.status === "downloading" && <div className="mt-3 h-2 overflow-hidden rounded-full bg-white"><div className="h-full rounded-full bg-[#1C4A44] transition-[width]" style={{ width: `${progress}%` }}/></div>}
        {availableNotes && desktopUpdate?.availableVersion && <details className="mt-3 overflow-hidden rounded-xl border bg-white text-left"><summary className="cursor-pointer px-3 py-2 text-xs font-black text-[#1C4A44]">Ver qué cambia en la versión {desktopUpdate.availableVersion}</summary><p className="max-h-48 overflow-y-auto whitespace-pre-line border-t px-3 py-3 text-xs leading-5 text-gray-600">{availableNotes}</p></details>}
      </div>
      {canManage && <div className="grid shrink-0 gap-2 sm:grid-cols-2">
        <button type="button" disabled={busy || ready} onClick={() => updates.check().catch(() => {})} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border bg-white px-4 text-xs font-bold text-gray-800 disabled:cursor-not-allowed disabled:opacity-45"><RefreshCw size={15} className={desktopUpdate?.status === "checking" ? "animate-spin" : ""}/>Buscar actualizaciones</button>
        <button type="button" disabled={!ready} onClick={() => updates.install().catch(() => {})} className="inline-flex min-h-11 items-center justify-center rounded-xl bg-green-700 px-4 text-xs font-bold text-white disabled:cursor-not-allowed disabled:bg-gray-300 disabled:text-gray-500">Reiniciar e instalar actualización</button>
      </div>}
    </div>
    {canManage && <p className="mt-3 text-xs leading-5 text-gray-500">El segundo botón se habilita cuando termina la descarga. Al usarlo, Kiosco+ se cierra, instala la versión y vuelve a abrirse automáticamente.</p>}
  </div>;
}
...
function ReferralCard({ account }) {
  ...
  const stats = account.referralStats || {};
  const copy = async () => {
    try {
  ...
  return <div className="min-w-0 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 sm:p-5"><div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-white text-[#1C4A44]"><Gift size={19}/></span><div className="min-w-0 flex-1"><h3 className="font-semibold">Tu código de referido</h3><p className="mt-1 text-xs leading-relaxed text-gray-600">Compartilo con otro comercio. Mientras mantenga su abono vigente, obtenés 20% de descuento.</p><div className="mt-3 flex flex-col gap-2 sm:flex-row"><code className="min-w-0 flex-1 rounded-lg border bg-white px-3 py-2 text-center text-sm font-bold tracking-wide sm:text-left">{account.referralCode}</code><button type="button" onClick={copy} className="flex min-h-10 items-center justify-center gap-2 rounded-lg bg-[#1C4A44] px-4 py-2 text-sm font-semibold text-white"><Copy size={15}/>{copied ? "Copiado" : "Copiar"}</button></div><div className="mt-3 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-white px-2.5 py-1 font-semibold text-emerald-800">{Number(stats.activeCount || 0)} activos</span>{Number(stats.pendingCount || 0) > 0 && <span className="rounded-full bg-white px-2.5 py-1 text-amber-700">{stats.pendingCount} pendientes</span>}{Number(stats.pausedCount || 0) > 0 && <span className="rounded-full bg-white px-2.5 py-1 text-gray-600">{stats.pausedCount} pausados</span>}<span className="rounded-full bg-white px-2.5 py-1 font-semibold text-[#1C4A44]">{Number(stats.automaticDiscountPercent ?? stats.discountPercent ?? 0)}% automático</span></div></div></div></div>;
}

function PlanStatusCard({ account }) {
  if (!account || account.superAdmin) return null;
  const access = trialAccessStatus(account);
  const paidCycles = paidBillingCycles(account);
  const introductoryRemaining = Math.max(0, INTRODUCTORY_PAID_MONTHS - paidCycles);
  const paymentsEnabled = import.meta.env.VITE_PAYMENTS_ENABLED === "1";
  const checkoutUrl = String(import.meta.env.VITE_MERCADOPAGO_CHECKOUT_URL || "").trim();
  const money = (value) => `$${Number(value).toLocaleString("es-AR")}`;
  const status = access.active
    ? `Beta activa · ${access.remainingDays || 1} día(s) disponibles`
    : access.reason === "subscription_expired"
      ? "Suscripción vencida · modo consulta"
...
export function SettingsModal({ preferences, onChange, customerDisplayPreferences = {}, onCustomerDisplayChange, promotions = [], products = [], onClose, cuenta, tenantId, onUpdateAccount, canEditBusiness = true, onCleanOperationalHistory, onExportCommercialArchive, archiveStats, syncStatus, initialSection = "apariencia", notificationPreviewMode = false, onReportProblem }) {
  const [section, setSection] = useState(initialSection || "apariencia");
  const [closing, setClosing] = useState(false);
  const [cloudConfig, setCloudConfig] = useState(loadCloudConfig());
  const [cloudTest, setCloudTest] = useState("");
  const [cloudUsername, setCloudUsername] = useState(cuenta?.superAdmin ? "kiosco-admin" : (cuenta?.usuario || ""));
  ...
  const cloudIsLocal = isLocalCloudApiUrl(cloudConfig.apiUrl);
  const cloudTargetName = cloudIsLocal ? "servidor local" : "nube de Render";
  ...
  const resetTutorials = () => {
    set("tutorialsCompleted", []);
    setTutorialReset(true);
    window.setTimeout(() => setTutorialReset(false), 2400);
  };
  ...
  useEffect(() => {
    ...
    let active = true;
    updates.getState().then((state) => { if (active) setDesktopUpdate(state); }).catch(() => {});
    const unsubscribe = updates.onState((state) => { if (active) setDesktopUpdate(state); });
    return () => { active = false; if (typeof unsubscribe === "function") unsubscribe(); };
  }, []);
  useEffect(() => {
    const updates = window.kioscoDesktop?.updates;
    if (!updates) return;
    updates.configure({ channel: cloudConfig.updateChannel, autoCheck: cloudConfig.autoCheckUpdates })
      .then(setDesktopUpdate)
      .catch(() => {});
  }, [cloudConfig.updateChannel, cloudConfig.autoCheckUpdates]);
  const saveCloud = (patch) => { const next = saveCloudConfig({ ...cloudConfig, ...patch }); setCloudConfig(next); window.dispatchEvent(new Event("kiosco-cloud-config-changed")); };
  const disconnectCloud = async () => { setCloudTest("Desconectando..."); try { await logoutCloud(cloudConfig.apiUrl); } finally { saveCloud({ enabled: false }); setCloudPassword(""); setCloudTest("Sesión de nube cerrada"); } };
  ...
  const connectCloud = async () => {
    setCloudTest("Conectando...");
    try {
      if (cuenta?.superAdmin) {
        await pairCloudDevice(cloudConfig.apiUrl, cloudPassword, cloudConfig.deviceId);
        saveCloud({ enabled: true });
        setCloudPassword("");
        setCloudTest("Dispositivo autorizado. La nube se conectará automáticamente.");
        return;
      }
      try {
        await loginCloud(cloudConfig.apiUrl, cloudUsername.trim(), cloudPassword, cloudConfig.deviceId);
      } catch (loginError) {
        if (loginError?.status !== 401) throw loginError;
        if (cuenta?.superAdmin) throw loginError;
        const bootstrap = await bootstrapCloud(cloudConfig.apiUrl, {
        ...
        if (!bootstrap.ok) {
          if (bootstrap.status === 409) throw loginError;
          const detail = await bootstrap.json().catch(() => ({}));
          throw new Error(detail.error || `No se pudo preparar la cuenta de nube (${bootstrap.status}).`);
        }
        await loginCloud(cloudConfig.apiUrl, cloudUsername.trim(), cloudPassword, cloudConfig.deviceId);
      }
      saveCloud({ enabled: true });
      setCloudPassword("");
      setCloudTest("Sesión de nube conectada");
    } catch (error) {
      setCloudTest(error?.message || "No se pudo iniciar sesión en la nube");
    }
  };
  const updateStatusText = desktopUpdate?.status === "checking"
    ? "Buscando una versión nueva..."
    : desktopUpdate?.status === "available"
      ? `Versión ${desktopUpdate.availableVersion} disponible. Preparando descarga...`
      : desktopUpdate?.status === "downloading"
        ? `Descargando versión ${desktopUpdate.availableVersion || "nueva"}: ${Math.round(desktopUpdate.percent || 0)}%`
        : desktopUpdate?.status === "installing"
          ? "Cerrando Kiosco+ para instalar la actualización y volver a abrirla..."
          : desktopUpdate?.status === "downloaded"
          ? `Versión ${desktopUpdate.availableVersion} lista para instalar.`
          : desktopUpdate?.status === "manual"
            ? "En Mac, descargá la versión nueva desde la página y volvé a copiarla a Aplicaciones. Tus datos no se borran."
            : desktopUpdate?.status === "store"
              ? "Instalada desde Microsoft Store. Windows administra las actualizaciones automáticamente."
            : desktopUpdate?.status === "up-to-date"
              ? "Este dispositivo ya tiene la última versión."
              : desktopUpdate?.status === "error"
                ? (desktopUpdate.error || "No se pudo comprobar la actualización.")
                : "Las actualizaciones se descargarán en segundo plano.";
  const groups = [
    { id: "negocio", label: "Negocio", icon: Store, sections: [["negocio", "Perfil y plan"]] },
    { id: "apariencia", label: "Apariencia", icon: Palette, sections: [["apariencia", "Colores"], ["interfaz", "Interfaz"], ["respuestas", "Sonido y movimiento"]] },
    { id: "notificaciones", label: "Notificaciones", icon: Bell, sections: [["notificaciones", "Preferencias"]] },
    { id: "operacion", label: "Funcionamiento", icon: SlidersHorizontal, sections: [["operacion", "Operación"], ["impresion", "Impresión y caja"]] },
    { id: "datos", label: "Datos y seguridad", icon: ShieldCheck, sections: [["datos", "Archivo"], ["seguridad", "Seguridad"], ["nube", "Nube y dispositivos"]] },
  ].filter((group) => group.id !== "negocio" || canEditBusiness);
  const activeGroup = groups.find((group) => group.sections.some(([id]) => id === section));
  const selectGroup = (group) => setSection(group.sections[0][0]);
  const bundledVersion = import.meta.env.VITE_APP_VERSION || "—";
  const displayedVersion = desktopUpdate?.currentVersion || bundledVersion;
  const hasBrowserWindow = typeof window !== "undefined";
  const isDesktopApp = hasBrowserWindow && Boolean(window.kioscoDesktop);
  const isInstalledWebApp = hasBrowserWindow && !isDesktopApp && (window.matchMedia?.("(display-mode: standalone)")?.matches || window.navigator?.standalone === true);
  const appModeLabel = isDesktopApp ? "Aplicación de escritorio" : isInstalledWebApp ? "Aplicación web instalada" : "Aplicación web";
  const copyDiagnostic = async () => {
    const diagnostic = [
      `Kiosco+ ${displayedVersion}`,
      appModeLabel,
      `Negocio: ${String(tenantId || "sin sesión")}`,
      `Dispositivo: ${cloudConfig.deviceId || "sin identificar"}`,
      `Nube: ${syncStatus?.mode || (cloudConnected ? "cloud" : "local")} / ${syncStatus?.state || "sin datos"}`,
      `Pendientes: ${Number(syncStatus?.pending || 0)} · Conflictos: ${Number(syncStatus?.conflicts || 0)}`,
      `Última sincronización: ${syncStatus?.lastSyncAt ? new Date(syncStatus.lastSyncAt).toLocaleString("es-AR") : "todavía no registrada"}`,
      `Conexión: ${typeof navigator === "undefined" || navigator.onLine ? "en línea" : "sin Internet"}`,
    ].join("\n");
    try { await navigator.clipboard.writeText(diagnostic); setDiagnosticCopied(true); setTimeout(() => setDiagnosticCopied(false), 1800); } catch { setCloudTest("No se pudo copiar el diagnóstico"); }
  };
  return <div className={`settings-overlay fixed inset-0 z-50 flex items-stretch justify-center bg-black/45 p-0 sm:items-center sm:p-4 ${closing ? "is-closing" : ""}`} onMouseDown={(event) => event.target === event.currentTarget && close()}>
    <div className="settings-panel flex h-[100dvh] w-full min-w-0 max-w-6xl flex-col overflow-hidden border bg-white shadow-2xl sm:h-[min(780px,94vh)] sm:flex-row sm:rounded-2xl" role="dialog" aria-modal="true" aria-label="Configuración de Kiosco+">
      <aside className="settings-navigation w-full shrink-0 border-b bg-gray-50 sm:w-60 sm:border-b-0 sm:border-r sm:p-4">
        <div className="settings-mobile-safe-header sm:hidden" aria-hidden="true" />
        <div className="settings-desktop-heading mb-5 hidden items-center gap-2 px-2 sm:flex"><Settings size={19}/><b>Configuración</b></div>
        <div className="settings-section-tabs grid grid-cols-2 gap-1 px-2 pb-2 sm:block sm:min-w-0 sm:space-y-1 sm:px-0 sm:pb-0">{groups.map((group) => { const Icon = group.icon; const active = activeGroup?.id === group.id; return <button key={group.id} onClick={() => selectGroup(group)} className={`flex min-w-0 items-center gap-2 rounded-lg px-3 py-2 text-left text-xs sm:w-full sm:text-sm ${active ? "bg-gray-900 text-white" : "text-gray-600 hover:bg-gray-100"}`}><Icon className="shrink-0" size={16}/><span className="truncate">{group.label}</span></button>; })}</div>
        <div className="border-t px-2 py-2 sm:mt-4 sm:px-0"><button type="button" onClick={() => setSection("ayuda")} className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs sm:text-sm ${section === "ayuda" ? "bg-gray-900 text-white" : "text-gray-600 hover:bg-gray-100"}`}><HelpCircle size={16}/>Ayuda y versión</button></div>
      </aside>
      <main className="min-w-0 flex-1 overflow-x-hidden overflow-y-auto p-4 sm:p-7"><div className="mb-4 flex items-start justify-between gap-3"><div className="min-w-0"><h2 className="break-words text-xl font-bold">{section === "ayuda" ? "Ayuda y versión" : activeGroup?.label}</h2><p className="break-words text-sm text-gray-500">{section === "ayuda" ? "Información útil para soporte y documentos de Kiosco+." : "Los cambios se guardan automáticamente para este usuario."}</p><p className="mt-1 break-words text-xs font-semibold text-gray-500">Kiosco+ v{displayedVersion} · {appModeLabel}</p></div><button onClick={close} className="shrink-0 rounded-lg p-2"><X size={20}/></button></div>
    {activeGroup?.sections.length > 1 && <div className="mb-5 flex flex-wrap gap-2 rounded-xl bg-gray-100 p-1">{activeGroup.sections.map(([id, label]) => <button type="button" key={id} onClick={() => setSection(id)} className={`min-h-9 flex-1 rounded-lg px-3 text-xs font-semibold ${section === id ? "bg-white text-gray-900 shadow-sm" : "text-gray-500"}`}>{label}</button>)}</div>}
    {section === "notificaciones" && <NotificationSettingsPanel preferences={current} onPreferencesChange={(patch) => onChange({ ...current, ...patch })} previewMode={notificationPreviewMode}/>}
    ...
    {section === "interfaz" && <div className="grid gap-3"><div className="rounded-xl border border-blue-200 bg-blue-50 p-3 text-xs leading-5 text-blue-900"><b>Ajustes para computadora</b><p>El ancho y el plegado del menú, junto con la cantidad de columnas, se aplican en pantallas grandes. En el celular la app conserva una distribución segura para no aplastar el contenido.</p></div><Field label="Densidad de la interfaz"><Select value={current.density} onChange={(v) => set("density", v)}><option value="comoda">Cómoda</option><option value="normal">Normal</option><option value="compacta">Compacta</option></Select></Field><Field label="Forma de controles"><Select value={current.rounded} onChange={(v) => set("rounded", v)}><option value="suave">Redondeados</option><option value="compacto">Más cuadrados</option></Select></Field><Field label="Tamaño del menú lateral"><Select value={current.sidebarSize} onChange={(v) => set("sidebarSize", v)}><option value="angosto">Angosto</option><option value="normal">Normal</option><option value="ancho">Ancho</option></Select></Field><Field label="Comportamiento del menú" hint="Si elegís Plegable, la app recuerda si lo dejaste abierto o cerrado."><Select value={current.sidebarMode} onChange={(v) => set("sidebarMode", v)}><option value="fijo">Siempre visible</option><option value="plegable">Plegable</option></Select></Field><Field label="Máximo de columnas en Inicio"><Select value={current.homeColumns} onChange={(v) => set("homeColumns", Number(v))}><option value="2">2 columnas</option><option value="3">3 columnas</option><option value="4">4 columnas</option></Select></Field><Field label="Ocultar valores sensibles" hint="Los importes quedan ocultos hasta desactivar esta opción; pasar el mouse ya no los revela."><Toggle value={current.hideSensitive} onChange={(v) => set("hideSensitive", v)}/></Field><Field label="Tutoriales de ayuda" hint="Hace que las guías vuelvan a aparecer al entrar en cada sección."><button type="button" onClick={resetTutorials} className={`min-h-10 w-full rounded-lg border px-3 py-2 text-sm font-semibold ${tutorialReset ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "hover:bg-gray-50"}`}>{tutorialReset ? "Listo: volverán a aparecer" : "Volver a mostrar"}</button></Field><Preview current={current}/></div>}
    ...
    {section === "operacion" && <>{!cuenta?.superAdmin && <CustomerDisplaySettings preferences={{ ...current, ...customerDisplayPreferences }} onChange={onCustomerDisplayChange || onChange} account={cuenta} businessId={tenantId} promotions={promotions} products={products} canConnectPayments={canEditBusiness}/>}<div className={`${cuenta?.superAdmin ? "" : "mt-4"} grid gap-3`}><Field label="Stock bajo predeterminado"><NumberField value={current.stockMinDefault} onChange={(v) => set("stockMinDefault", v)}/></Field><Field label="Alerta de vitrina predeterminada"><NumberField value={current.vitrinaAlertDefault} onChange={(v) => set("vitrinaAlertDefault", v)}/></Field><Field label="Margen de ganancia objetivo"><NumberField value={current.targetMargin} suffix="%" onChange={(v) => set("targetMargin", v)}/></Field><Field label="Descuento máximo sin autorización"><NumberField value={current.maxDiscount} max={100} suffix="%" onChange={(v) => set("maxDiscount", v)}/></Field><Field label="Método de pago predeterminado"><Select value={current.defaultPayment} onChange={(v) => set("defaultPayment", v)}><option>Efectivo</option><option>Mercado Pago</option><option>Transferencia</option><option>Tarjeta</option><option>Cuenta corriente</option></Select></Field><Field label="Cámara preferida"><Select value={current.preferredCamera} onChange={(v) => set("preferredCamera", v)}><option value="automatica">Automática</option><option value="frontal">Frontal</option><option value="trasera">Trasera</option></Select></Field><Field label="Permitir stock negativo"><Toggle value={current.allowNegativeStock} onChange={(v) => set("allowNegativeStock", v)}/></Field><Field label="Días para alertar vencimientos"><NumberField value={current.expiryDays} onChange={(v) => set("expiryDays", v)}/></Field><Field label="Mostrar número en tickets nuevos"><Toggle value={current.ticketNumbering} onChange={(v) => set("ticketNumbering", v)}/></Field><Field label="Prefijo de tickets nuevos"><input value={current.ticketPrefix} onChange={(e) => set("ticketPrefix", e.target.value.slice(0, 8))} placeholder="Ej.: KS-" className="rounded-lg border px-3 py-2 text-sm"/></Field><Field label="Redondeo de precios"><Select value={current.rounding} onChange={(v) => set("rounding", v)}><option value="centavos">Con centavos</option><option value="unidad">Al peso</option><option value="decena">A la decena</option><option value="centena">A la centena</option></Select></Field></div></>}
    {section === "seguridad" && <div className="grid gap-3"><Field label="Inactividad antes de cerrar sesión" hint="Cero mantiene la sesión activa hasta su duración máxima."><NumberField value={current.inactivityMinutes} suffix="min" onChange={(v) => set("inactivityMinutes", v)}/></Field><Field label="Duración máxima de sesión"><NumberField value={current.sessionHours} min={1} max={24} suffix="h" onChange={(v) => set("sessionHours", v)}/></Field><Field label="Ventana de tickets duplicados"><NumberField value={current.duplicateHours} min={1} max={168} suffix="h" onChange={(v) => set("duplicateHours", v)}/></Field><Field label="Pedir motivo para anulaciones y devoluciones"><Toggle value={current.requireCorrectionReason} onChange={(v) => set("requireCorrectionReason", v)}/></Field><Field label="Confirmar anulaciones desde Reportes"><Toggle value={current.confirmDangerousActions} onChange={(v) => set("confirmDangerousActions", v)}/></Field></div>}
    {section === "nube" && <div className="grid min-w-0 gap-3">
      <div className="min-w-0 rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-800 sm:p-4"><div className="flex items-center justify-between gap-2"><b>Sincronización local-first</b><span className="rounded-full bg-white px-2 py-1 text-[10px] font-bold uppercase">{cloudIsLocal ? "Local" : "Render"}</span></div><p className="mt-1 break-words">La app guarda primero en este dispositivo y después envía los cambios a {cloudTargetName}.</p></div>
      {cloudConfig.migratedFromLocal && !cloudIsLocal && !cloudConnected && <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><b>Conexión corregida</b><p className="mt-1 text-xs">Esta instalación estaba apuntando al servidor local. Ahora usa Render. Si la sesión todavía no se conectó, cerrá tu sesión de Kiosco+ y volvé a entrar una vez.</p></div>}
      <Field label="Activar sincronización"><Toggle value={cloudConfig.enabled} onChange={(v)=>saveCloud({enabled:v})}/></Field>
      <Field label="Dirección del servidor"><input value={cloudConfig.apiUrl} onChange={(e)=>saveCloud({apiUrl:e.target.value})} placeholder="https://api.tudominio.com" className="min-w-0 rounded-lg border px-3 py-2 text-sm"/></Field>
      <Field label="Identificador del dispositivo" hint="Permite reconocer desde qué dispositivo llegó cada cambio."><code className="truncate rounded-lg bg-gray-100 px-3 py-2 text-xs">{cloudConfig.deviceId}</code></Field>
      {desktopUpdate?.status !== "store" && <Field label="Canal de actualizaciones"><Select value={cloudConfig.updateChannel} onChange={(v)=>saveCloud({updateChannel:v})}><option value="stable">Estable</option><option value="beta">Pruebas</option></Select></Field>}
      {desktopUpdate?.status !== "store" && <Field label="Comprobar actualizaciones automáticamente"><Toggle value={cloudConfig.autoCheckUpdates} onChange={(v)=>saveCloud({autoCheckUpdates:v})}/></Field>}
      <DesktopUpdateCard desktopUpdate={desktopUpdate} displayedVersion={displayedVersion} appModeLabel={appModeLabel} updateStatusText={updateStatusText}/>
      <div className="flex flex-col items-stretch gap-2 sm:flex-row sm:items-center sm:justify-end sm:gap-3"><span className={`break-words text-sm ${cloudTest.includes("disponible")?"text-green-600":cloudTest.includes("No")?"text-red-600":"text-gray-500"}`}>{cloudTest}</span><button disabled={!cloudConfig.apiUrl} onClick={testCloud} className="min-h-11 w-full rounded-lg bg-gray-900 px-4 py-2 text-sm text-white disabled:opacity-40 sm:min-h-0 sm:w-auto">Probar conexión</button></div>
    </div>}
    {section === "nube" && <div className="mt-4 min-w-0 rounded-xl border p-3 sm:p-4"><h3 className="text-sm font-semibold">{cuenta?.superAdmin ? "Conectar este dispositivo" : "Sesión de la nube"}</h3><p className="mt-1 break-words text-xs text-gray-500">{cuenta?.superAdmin ? <>Ingresá una sola vez la clave privada configurada en Render. Después este dispositivo se conectará automáticamente.</> : <>Usá el mismo usuario y contraseña de Kiosco+ para conectar este dispositivo con {cloudTargetName}.</>}</p><div className={`mt-3 grid min-w-0 gap-2 ${cuenta?.superAdmin ? "" : "sm:grid-cols-2"}`}>{!cuenta?.superAdmin&&<input value={cloudUsername} onChange={(e)=>setCloudUsername(e.target.value)} placeholder="Usuario" className="min-w-0 rounded-lg border px-3 py-2 text-sm"/>}<input type="password" value={cloudPassword} onChange={(e)=>setCloudPassword(e.target.value)} placeholder={cloudConnected?"Dispositivo conectado":cuenta?.superAdmin?"Clave privada del dispositivo":"Contraseña"} className="min-w-0 rounded-lg border px-3 py-2 text-sm"/></div><div className="mt-3 flex justify-end"><button disabled={!cloudConfig.apiUrl||(!cuenta?.superAdmin&&!cloudUsername)||!cloudPassword} onClick={connectCloud} className="min-h-11 w-full rounded-lg bg-blue-600 px-4 py-2 text-sm text-white disabled:opacity-40 sm:min-h-0 sm:w-auto">{cuenta?.superAdmin?"Autorizar este dispositivo":`Conectar con ${cloudTargetName}`}</button></div></div>}
    {section === "nube" && cloudConnected && <div className="mt-3 flex min-w-0 flex-col items-stretch gap-3 rounded-xl border border-green-200 bg-green-50 p-3 sm:flex-row sm:items-center sm:justify-between sm:p-4"><div className="min-w-0"><b className="text-sm text-green-800">{cloudIsLocal ? "Servidor local conectado" : "Nube de Render conectada"}</b><p className="break-words text-xs text-green-700">Esta sesión puede sincronizar los datos de este dispositivo.</p></div><button onClick={disconnectCloud} className="min-h-11 w-full rounded-lg border border-red-200 bg-white px-4 py-2 text-sm text-red-600 sm:min-h-0 sm:w-auto">Cerrar sesión de nube</button></div>}
    {section === "negocio" && <div className="grid min-w-0 gap-4"><ReferralCard account={cuenta}/><div className="min-w-0 rounded-2xl border p-4 sm:p-5"><div className="flex flex-col items-center gap-4 sm:flex-row sm:items-center sm:gap-5"><div className="grid h-24 w-24 shrink-0 place-items-center overflow-hidden rounded-2xl border bg-gray-50">{cuenta?.imagenNegocio ? <img src={cuenta.imagenNegocio} alt="Imagen del negocio" className="h-full w-full object-contain"/> : <Store size={34} className="text-gray-400"/>}</div><div className="min-w-0 w-full flex-1 text-center sm:text-left"><h3 className="font-semibold">Imagen del negocio</h3><p className="mt-1 break-words text-xs text-gray-500">La imagen se reduce automáticamente para que siga siendo liviana.</p><div className="mt-3 grid grid-cols-1 gap-2 min-[380px]:grid-cols-2 sm:flex sm:flex-wrap"><label className="flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-lg bg-gray-900 px-3 py-2 text-sm text-white sm:min-h-0"><ImagePlus size={16}/>{cuenta?.imagenNegocio ? "Reemplazar imagen" : "Subir imagen"}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={selectBusinessImage} className="hidden"/></label>{cuenta?.imagenNegocio && <button type="button" onClick={() => onUpdateAccount?.({ imagenNegocio: null })} className="flex min-h-11 items-center justify-center gap-2 rounded-lg border border-red-200 px-3 py-2 text-sm text-red-600 sm:min-h-0"><Trash2 size={15}/>Quitar</button>}</div>{businessImageError && <p className="mt-2 break-words text-xs text-red-600">{businessImageError}</p>}</div></div></div><Field label="Nombre visible del negocio" hint="Aparece en el menú lateral y en las pantallas del negocio."><input value={businessName} onChange={(event) => setBusinessName(event.target.value)} onBlur={saveBusinessName} onKeyDown={(event) => { if (event.key === "Enter") { saveBusinessName(); event.currentTarget.blur(); } }} maxLength={60} className="min-w-0 rounded-lg border px-3 py-2 text-sm"/></Field><Field label="Forma de trabajo" hint="Podés cambiarla después. Los empleados guardados no se borran al elegir Trabajo solo."><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => onUpdateAccount?.({ modoNegocio: "solo" })} className={`min-h-11 rounded-lg border px-2 text-xs font-semibold ${cuenta?.modoNegocio !== "equipo" ? "border-blue-600 bg-blue-50 text-blue-800" : ""}`}>Trabajo solo</button><button type="button" onClick={() => onUpdateAccount?.({ modoNegocio: "equipo" })} className={`min-h-11 rounded-lg border px-2 text-xs font-semibold ${cuenta?.modoNegocio === "equipo" ? "border-blue-600 bg-blue-50 text-blue-800" : ""}`}>Tengo empleados</button></div></Field><div className="min-w-0 rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-800 sm:p-4"><b>Guardado por cuenta</b><p className="mt-1 break-words">Esto cambia la identidad del negocio, no el nombre personal del dueño ni sus credenciales.</p></div></div>}
    ...
    {section === "datos" && <div className="grid gap-3"><DataRecoveryPanel account={cuenta} enabled={canEditBusiness && !cuenta?.superAdmin}/><div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900"><b>Dos historiales separados</b><p className="mt-1">La limpieza sólo alcanza recordatorios, listas completadas y compras ya recibidas. Ventas, caja, auditoría y comprobantes quedan protegidos.</p></div><Field label="Limpieza automática del historial operativo"><Toggle value={current.automaticOperationalCleanup} onChange={(value)=>set("automaticOperationalCleanup",value)}/></Field><Field label="Conservar historial operativo"><Select value={current.operationalHistoryRetentionMonths} onChange={(value)=>set("operationalHistoryRetentionMonths",Number(value))}><option value="3">3 meses</option><option value="6">6 meses</option><option value="12">12 meses</option><option value="24">24 meses</option></Select></Field><Field label="Archivo comercial protegido" hint={`${archiveStats?.count || 0} comprobante(s). Se guardan como datos livianos y se imprimen o convierten a PDF cuando se necesitan.`}><span className="text-sm font-semibold">Mínimo 10 años</span></Field><div className="grid gap-2 sm:grid-cols-2"><button type="button" onClick={()=>onCleanOperationalHistory?.(current.operationalHistoryRetentionMonths)} className="flex min-h-11 items-center justify-center gap-2 rounded-lg border px-3 text-sm font-semibold"><Trash2 size={15}/>Limpiar ahora</button><button type="button" onClick={()=>onExportCommercialArchive?.()} className="flex min-h-11 items-center justify-center gap-2 rounded-lg bg-gray-900 px-3 text-sm font-semibold text-white"><Download size={15}/>Exportar archivo comercial</button></div><p className="text-xs text-gray-500">Los documentos internos de Kiosco+ siguen marcados como NO FISCAL · SIN CAE. Este archivo no reemplaza facturas oficiales emitidas ante ARCA.</p></div>}
    {section === "negocio" && <PlanStatusCard account={cuenta}/>}
    {section === "nube" && <div className="mt-4 min-w-0 rounded-xl border border-dashed p-3 sm:p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><h3 className="text-sm font-bold">Diagnóstico para soporte</h3><p className="mt-1 text-xs text-gray-500">Incluye versión, tipo de app, identificadores técnicos y estado de sincronización. No incluye contraseñas ni datos de ventas.</p><div className="mt-3 grid gap-1 text-xs text-gray-600 sm:grid-cols-2"><span>Estado: <b>{syncStatus?.state || "sin datos"}</b></span><span>Cambios pendientes: <b>{Number(syncStatus?.pending || 0)}</b></span><span>Conflictos: <b>{Number(syncStatus?.conflicts || 0)}</b></span><span>Última sincronización: <b>{syncStatus?.lastSyncAt ? new Date(syncStatus.lastSyncAt).toLocaleString("es-AR") : "—"}</b></span><span className="break-all sm:col-span-2">Dispositivo: <b>{cloudConfig.deviceId || "—"}</b></span></div></div><button type="button" onClick={copyDiagnostic} className="inline-flex min-h-10 shrink-0 items-center justify-center gap-2 rounded-lg border bg-white px-3 text-xs font-semibold"><Copy size={15}/>{diagnosticCopied ? "Copiado" : "Copiar diagnóstico"}</button></div></div>}
    {section === "ayuda" && <div className="grid gap-4"><DesktopUpdateCard desktopUpdate={desktopUpdate} displayedVersion={displayedVersion} appModeLabel={appModeLabel} updateStatusText={updateStatusText}/><ReleaseNotesHistory/><div className="grid gap-3 sm:grid-cols-2"><button type="button" onClick={() => set("tutorialsCompleted", [])} className="min-h-12 rounded-xl border bg-white px-4 text-sm font-semibold">Volver a mostrar tutoriales</button><button type="button" onClick={copyDiagnostic} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border bg-white px-4 text-sm font-semibold"><Copy size={16}/>{diagnosticCopied ? "Diagnóstico copiado" : "Copiar diagnóstico"}</button>{onReportProblem && <button type="button" onClick={() => { close(); setTimeout(() => onReportProblem(), 180); }} className="min-h-12 rounded-xl bg-[#1C4A44] px-4 text-sm font-bold text-white">Informar un problema</button>}<a href="./terminos.html" target="_blank" rel="noopener noreferrer" className="flex min-h-12 items-center justify-center rounded-xl border bg-white px-4 text-sm font-semibold">Términos y condiciones</a><a href="./privacidad.html" target="_blank" rel="noopener noreferrer" className="flex min-h-12 items-center justify-center rounded-xl border bg-white px-4 text-sm font-semibold">Política de privacidad</a></div><p className="text-xs leading-5 text-gray-500">El diagnóstico incluye versión, dispositivo y estado de sincronización. No contiene contraseñas ni datos de ventas.</p></div>}
  </main></div></div>;
}
...
export function applyPreferences(preferences = {}) {
  ...
  root.dataset.sidebarMode = value.sidebarMode;
