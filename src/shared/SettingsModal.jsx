...
function ReferralCard({ account }) {
  ...
  return <div className="min-w-0 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 sm:p-5"><div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-white text-[#1C4A44]"><Gift size={19}/></span><div className="min-w-0 flex-1"><h3 className="font-semibold">Tu código de referido</h3><p className="mt-1 text-xs leading-relaxed text-gray-600">Compartilo con otro comercio. Cuando active su primer abono, obtenés 20% de descuento.</p><div className="mt-3 flex flex-col gap-2 sm:flex-row"><code className="min-w-0 flex-1 rounded-lg border bg-white px-3 py-2 text-center text-sm font-bold tracking-wide sm:text-left">{account.referralCode}</code><button type="button" onClick={copy} className="flex min-h-10 items-center justify-center gap-2 rounded-lg bg-[#1C4A44] px-4 py-2 text-sm font-semibold text-white"><Copy size={15}/>{copied ? "Copiado" : "Copiar"}</button></div><div className="mt-3 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-white px-2.5 py-1 font-semibold text-emerald-800">{Number(stats.activeCount || 0)} activos</span>{Number(stats.pendingCount || 0) > 0 && <span className="rounded-full bg-white px-2.5 py-1 text-amber-700">{stats.pendingCount} pendientes</span>}<span className="rounded-full bg-white px-2.5 py-1 font-semibold text-[#1C4A44]">{Number(stats.discountPercent || 0)}% de descuento</span></div></div></div></div>;
}
