...
export function Sidebar({ current, onNavigate, cuenta, identidad, permisos, onLogout, products, data, preferences = {}, onReturnAdmin, menuOrder = [], onMenuOrderChange, onOpenSettings, onReportProblem, onGlobalScan, onHelp, syncStatus, onSyncNow, demoMode = false }) {
  ...
  const syncTargetName = syncStatus?.serverMode === "local" ? "servidor local" : "nube de Render";
  const syncButtonLabel = syncStatus?.mode !== "cloud"
    ? "Datos locales"
      ...
      : syncStatus?.state === "error"
        ...
        : syncStatus?.state === "offline"
          ...
          : syncStatus?.conflicts
            ? `${syncStatus.conflicts} conflicto(s) para revisar`
            : syncStatus?.pending
              ? `${syncStatus.pending} cambio(s) pendiente(s)`
              : syncStatus?.serverMode === "local"
                ? "Servidor local sincronizado"
                : "Nube de Render sincronizada";
  useEffect(() => {
  ...
  const conflictTitle = (conflict) => {
    ...
    if (conflict?.entity === "cajaEstado") return entityLabel(conflict.entity);
  ...
  const discardPendingBatch = async () => {
    const pendingIds = syncReview.pending.map((operation) => operation.id);
    for (const operationId of pendingIds) await syncEngine.discardPending(operationId);
    setDiscardPendingBatchOpen(false);
  ...
  return (
    <div className={`app-sidebar h-full min-h-0 w-56 shrink-0 overflow-hidden border-r border-gray-200 bg-white flex flex-col ${collapsed ? "app-sidebar--collapsed" : ""}`}>
      ...
      <div className="sidebar-footer relative shrink-0 px-4 py-4 border-t border-gray-100 text-sm">
        ...
        <button onClick={openSyncReview} aria-expanded={syncReviewOpen} title={syncStatus?.error || `Estado de sincronización: ${syncTargetName}`} className="mb-3 flex w-full min-w-0 items-center gap-2 rounded-lg border px-2 py-1.5 text-left text-xs"><span className={`h-2 w-2 shrink-0 rounded-full ${syncStatus?.state === "error" ? "bg-red-500" : syncStatus?.state === "conflict" ? "bg-purple-500" : syncStatus?.state === "offline" ? "bg-amber-500" : syncStatus?.state === "syncing" ? "animate-pulse bg-blue-500" : "bg-green-500"}`}/><span className="min-w-0 flex-1 truncate">{syncButtonLabel}</span>{(syncStatus?.conflicts||syncStatus?.pending)>0&&<ChevronRight size={13} className={`shrink-0 transition-transform ${syncReviewOpen?"-rotate-90":"rotate-90"}`}/>}</button>
        ...
        {trial.active && <p className="mt-2 rounded-lg border border-amber-200 bg-amber-50 px-2 py-1.5 text-[10px] font-semibold leading-4 text-amber-900">Prueba activa<br/>Vence {formatTrialExpiration(cuenta)}</p>}
