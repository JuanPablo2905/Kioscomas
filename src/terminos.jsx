  ...
  return <main>
    ...
    <div className="legal-shell">
      ...
      <article className="legal-document">
        ...
        <Section number={1} title="Identificación del proveedor"><p>El servicio se denomina <b>“Kiosco+”</b> y es ofrecido por <b>{provider.name || "[COMPLETAR NOMBRE O RAZÓN SOCIAL]"}</b>, CUIT/CUIL <b>{provider.cuit || "[COMPLETAR CUIT O CUIL]"}</b>, con domicilio en <b>{provider.address || "[COMPLETAR DOMICILIO]"}</b> y correo de atención <ContactEmail email={provider.email} fallback="[COMPLETAR CORREO]"/>. En estos Términos, <b>{provider.name || "el titular del servicio"}</b> será denominado <b>“el Proveedor”</b>.</p><p>Canal adicional de atención: WhatsApp <a href={whatsappUrl("Hola Kiosco+, necesito realizar una consulta.")} target="_blank" rel="noopener noreferrer">+{whatsappNumber}<ExternalLink size={13}/></a>.</p></Section>
        ...
        <Section number={5} title="Planes, precios, facturación y pagos"><p>El precio de lista del plan Kiosco+ es de $50.000 mensuales por negocio e incluye dos dispositivos simultáneos, salvo una oferta particular informada antes de contratar. Cada dispositivo adicional tiene el valor publicado en la página de precios.</p><p>Durante el lanzamiento, los primeros tres períodos mensuales pagos tienen un valor promocional de $30.000 cada uno. Desde el cuarto período pago rige el precio de lista, sujeto a descuentos vigentes. La contratación paga sólo comienza después de que el Usuario confirma que desea continuar.</p><p>La suscripción se habilita por el período abonado. Al vencer, Kiosco+ puede pasar a modo consulta hasta la renovación: los datos continúan visibles y exportables, pero pueden bloquearse nuevas modificaciones.</p><p>No existe permanencia mínima ni débito automático durante la beta. Los cambios de precio se informarán antes de aplicarse al siguiente período.</p></Section>

        <Section number={6} title="Pruebas, promociones y referidos"><p>Las cuentas nuevas habilitadas para la beta reciben 30 días de uso sin cargo y sin registrar una tarjeta. Finalizado ese período, el Usuario puede decidir no continuar; sus datos podrán quedar temporalmente disponibles en modo consulta conforme a la política de conservación.</p><p>En el programa de referidos, cada comercio referido comienza a computar cuando registra su primer pago válido y conserva el beneficio únicamente mientras su abono está vigente. El descuento acumulado se aplica desde el cuarto período pago del comercio que refiere. Si el abono del referido vence, el beneficio se pausa y vuelve cuando renueva. Los beneficios pueden anularse ante pagos revertidos, cuentas duplicadas, fraude, auto-referidos o abuso del sistema.</p></Section>
        ...
        <Section number={12} title="Privacidad y datos personales"><p>Para crear y operar cuentas pueden tratarse datos de identificación, contacto, negocio, usuarios autorizados, dispositivos, seguridad, soporte y actividad de sincronización. La finalidad es prestar el servicio, autenticar accesos, prevenir fraude, brindar soporte y cumplir obligaciones legales.</p><p>El correo electrónico puede utilizarse para mensajes transaccionales necesarios, como bienvenida, habilitación de la cuenta, recuperación de acceso y alertas de seguridad. Para entregarlos, el dato se comunica al proveedor de correo contratado; no se envían contraseñas por email.</p><p>El Usuario debe contar con una base legal adecuada para cargar datos de empleados, clientes o terceros. Podrá solicitar acceso, rectificación, actualización o supresión de sus datos personales mediante el correo de atención indicado en la cláusula 1, sujeto a las excepciones y plazos legales.</p><p>No se comercializan datos personales como producto. La infraestructura puede apoyarse en proveedores tecnológicos, actualmente incluyendo servicios de alojamiento, base de datos, correo transaccional y distribución de software, bajo medidas razonables de seguridad y acceso restringido.</p></Section>
    ...
    <footer className="legal-footer"><img src={`${base}kiosco-plus-lockup-principal.svg`} alt="Kiosco+"/><span>Versión contractual {TERMS_VERSION}</span><a href="./privacidad.html">Política de privacidad</a><a href="./">Volver a Kiosco+</a></footer>
  </main>;
}
