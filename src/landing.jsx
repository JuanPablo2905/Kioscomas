...
const preferredDownload = visitorUsesMobile
  ...
  : { url: "./descargas.html", label: visitorUsesMac ? "Descargas para Mac" : "Descargas" };

const features = [
  { icon: MonitorUp, title: "Una segunda pantalla de verdad", text: "Mostrá la compra al cliente o armá una pantalla publicitaria remota con widgets libres.", href: "./funciones.html#pantallas" },
  { icon: WifiOff, title: "Seguís trabajando sin Internet", text: "Ventas, caja y stock quedan guardados en el dispositivo y se sincronizan al volver la conexión.", href: "./funciones.html#offline" },
  { icon: CreditCard, title: "Cobros en una sola venta", text: "Efectivo, tarjeta, transferencia, cuenta corriente, pago combinado y Mercado Pago sin cambiar de pantalla.", href: "./funciones.html#ventas" },
  { icon: Package, title: "Depósito y vitrina separados", text: "Sabé qué queda guardado, qué está exhibido, qué vencerá y qué conviene reponer.", href: "./funciones.html#stock" },
  { icon: Users, title: "Cada persona con sus permisos", text: "Usuarios, roles y un historial legible de quién hizo cada operación importante.", href: "./funciones.html#equipo" },
  { icon: BarChart3, title: "Decisiones sin planillas", text: "Ventas, margen, caja y productos quietos explicados con información fácil de leer.", href: "./funciones.html#reportes" },
];
...
function App() {
  ...
  return <main>
    ...
    <section className="hero" id="inicio"><div className="hero-copy hero-enter"><div className="eyebrow"><Sparkles size={15}/> Hecha para comercios reales</div><h1>Tu negocio, <i>más claro</i> todos los días.</h1><p>Stock, ventas, caja, compras y clientes en una sola herramienta simple de usar. Pensada para kioscos y comercios de barrio.</p><div className="hero-actions" style={{flexWrap:"wrap"}}><a className="button primary" href={cloudAppUrl}>Ingresar a mi cuenta <ArrowRight size={18}/></a><a className="button ghost" href={demoUrl}>Probar demo <ArrowRight size={18}/></a><a className="button ghost" href="./descargas.html"><Download size={18}/> Descargas</a></div><div className="hero-trust"><span><Check size={15}/> Windows 10 y 11</span><span><Check size={15}/> Mac Intel y Apple Silicon</span><span><Check size={15}/> Celular y computadora</span><span><Check size={15}/> Hecha en Argentina</span></div><a className="hero-mp-link" href="#mercado-pago"><QrCode size={17}/><span><b>Mercado Pago:</b> QR estático disponible; QR dinámico y Point en integración.</span><ArrowRight size={15}/></a></div><div className="hero-visual hero-device-enter"><div className="glow"/><DemoPanel/><div className="demo-hint"><span><Sparkles size={14}/> Tocá las secciones para recorrer la app</span><a href={demoUrl}>Abrir la demo completa <ArrowRight size={15}/></a></div></div></section>
    ...
    <section className="section features" id="funciones"><div className="section-intro reveal"><span className="eyebrow">Lo que distingue a Kiosco+</span><h2>Funciones pensadas para el mostrador real.</h2><p>Acá tenés el resumen. Tocá cualquier tarjeta para conocer el funcionamiento completo y qué necesitás para usarlo.</p></div><div className="feature-grid">{features.map(({icon: Icon, title, text, href}, index)=><a className="feature reveal" href={href} style={{"--delay":`${index * 80}ms`}} key={title}><div className="feature-icon"><Icon size={22}/></div><h3>{title}</h3><p>{text}</p><span className="feature-more">Ver detalle <ArrowRight size={15}/></span></a>)}</div><a className="section-more-link" href="./funciones.html">Ver todas las funciones de Kiosco+ <ArrowRight size={17}/></a></section>
