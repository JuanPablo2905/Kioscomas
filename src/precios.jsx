...
function App() {
  ...
  return <main>
    ...
    <section className="section plans" id="planes"><div className="plans-grid">
      <article className={"plan-card reveal " + (hoverPlans ? "launch" : "main") + (hoverPlans === "main" ? " hovered" : "")} onMouseEnter={() => setHoverPlans("main")} onMouseLeave={() => setHoverPlans(null)}><span className="plan-badge">Lo que elige la mayoría</span><div className="plan-icon"><Store size={22}/></div><h3>Kiosco+</h3><div className="plan-price"><b>$30.000</b><span>/mes</span></div><p className="plan-note">Incluye <b>2 dispositivos simultáneos</b>. Cada dispositivo adicional suma <b>$5.000/mes</b>.</p><ul>{planFeatures.map((item)=><li key={item}><Check size={15}/>{item}</li>)}</ul><a className="button primary" href={wa("Hola Kiosco+, me interesa el plan de $30.000/mes con 2 dispositivos incluidos. ¿Cómo lo contrato?")} target="_blank" rel="noopener noreferrer">Quiero este plan <ArrowRight size={18}/></a></article>

      <article className={"plan-card reveal " + (hoverPlans ? "main" : "launch") + (hoverPlans === "launch" ? " hovered" : "")} onMouseEnter={() => setHoverPlans("launch")} onMouseLeave={() => setHoverPlans(null)}><span className="plan-badge">Lanzamiento</span><div className="plan-icon"><Sparkles size={22}/></div><h3>Plan Lanzamiento</h3><div className="plan-price"><b>$0</b><span>el primer mes</span></div><p className="plan-note">Después queda en <b>$20.000/mes por siempre</b>, como agradecimiento por ayudarnos a cerrar la app.</p><ul>{planFeatures.map((item)=><li key={item}><Check size={15}/>{item}</li>)}</ul><a className="button ghost" href={wa("Hola Kiosco+, me interesa el Plan Lanzamiento (primer mes gratis, después $20.000/mes). ¿Cómo lo contrato?")} target="_blank" rel="noopener noreferrer">Quiero el plan lanzamiento <ArrowRight size={18}/></a></article>
    </div></section>
