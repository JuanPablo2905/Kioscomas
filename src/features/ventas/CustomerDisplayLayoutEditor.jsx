...
function WidgetVisual({ widget, state }) {
  ...
  if (type === "social") { const platform = SOCIAL_PLATFORMS[widget.platform] || SOCIAL_PLATFORMS.web; return <div className="flex h-full w-full flex-col items-center justify-center overflow-hidden rounded-md p-1.5 text-center" style={{ background: widget.platform === "instagram" ? "linear-gradient(135deg,#833AB4,#FD1D1D,#FCAF45)" : platform.color, color: platform.foreground }}><b className="truncate text-[6px] uppercase sm:text-[8px]">{widget.label || platform.label}</b><p className="max-w-full truncate text-[5px] font-semibold sm:text-[7px]">{widget.value || "Completá el dato"}</p><span className="mt-1 grid aspect-square h-[32%] place-items-center rounded bg-white text-[4px] font-black text-emerald-950 sm:text-[6px]">QR</span></div>; }
