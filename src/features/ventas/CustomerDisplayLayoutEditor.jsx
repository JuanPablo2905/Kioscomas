...
function WidgetVisual({ widget, state }) {
  ...
  if (type === "social") { const platform = SOCIAL_PLATFORMS[widget.platform] || SOCIAL_PLATFORMS.web; const label = widget.label || platform.label; const value = widget.value || "Completá el dato"; const textLength = `${label} ${value}`.trim().length; const textDensity = textLength > 54 ? "long" : textLength > 32 ? "medium" : "short"; return <div className="customer-social-card rounded-md" data-text-density={textDensity} style={{ background: widget.platform === "instagram" ? "linear-gradient(135deg,#833AB4,#FD1D1D,#FCAF45)" : platform.color, color: platform.foreground }}><div className="customer-social-card__text"><b className="customer-social-card__title">{label}</b><p className="customer-social-card__value">{value}</p></div><span className="customer-social-card__qr grid place-items-center font-black text-emerald-950">QR</span></div>; }
...
function VisualLayoutEditor({ config, mode, previewState, onChange }) {
  ...
  return <div className="grid gap-3">
    ...
    <div className="rounded-xl border bg-white p-3"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-black">Agregar un bloque</p><p className="mt-0.5 text-[10px] text-gray-500">Cada red social se crea como un cuadro independiente. Después podés moverla, estirarla o quitarla sin afectar a las demás.</p></div><button type="button" onClick={createSocial} className="inline-flex min-h-10 shrink-0 items-center justify-center gap-1.5 rounded-lg bg-[#1C4A44] px-3 text-[10px] font-bold text-white"><Plus size={14}/>Nueva red o contacto</button></div>{available.length ? <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">{available.map((item) => <button type="button" key={item.id} onClick={() => add(item)} className="flex min-h-12 items-center justify-center rounded-xl border bg-gray-50 px-2 text-center text-[10px] font-bold text-gray-800 transition hover:border-emerald-600 hover:bg-emerald-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600">{widgetName(item)}</button>)}</div> : <p className="mt-3 rounded-lg bg-emerald-50 px-3 py-2 text-center text-[10px] font-semibold text-emerald-800">Los bloques existentes ya están en esta vista. Podés crear otra red con el botón de arriba.</p>}</div>
