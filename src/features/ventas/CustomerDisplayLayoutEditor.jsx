...
function WidgetVisual({ widget, state }) {
  ...
  if (type === "hours") return <div className="rounded-md bg-white/10 p-1.5"><b className="text-[6px] text-amber-300 sm:text-[8px]">HORARIOS</b><p className="line-clamp-2 text-[5px] sm:text-[7px]">{widget.text || "Lun a sáb · 8 a 20 h"}</p></div>;
...
function PreviewBlock({ id, widget, state, selected, onSelect, onDragStart }) {
  return <button type="button" draggable onDragStart={() => onDragStart(id)} onClick={(event) => { event.stopPropagation(); onSelect(id); }} className={`group relative min-w-0 rounded-lg border p-0.5 text-left transition ${selected ? "border-amber-300 ring-2 ring-amber-300/70" : "border-white/15 hover:border-white/60"}`}>
...
function VisualZone({ zone, ids, config, state, selectedId, onSelect, draggingId, onDragStart, onDrop, horizontal = false }) {
  return <div onClick={() => onSelect("")} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); if (draggingId) onDrop(draggingId, zone); }} className={`relative min-h-0 min-w-0 rounded-md border border-dashed border-white/15 bg-black/5 p-1 ${horizontal ? "flex-none" : "h-full"}`}>
    ...
    <div className={`${horizontal ? "flex min-w-0 gap-1 pt-1" : "grid max-h-full gap-1 overflow-hidden pt-1"}`}>{ids.map((id) => <div key={id} className="min-w-0 flex-1"><PreviewBlock id={id} widget={config.widgets[id]} state={state} selected={selectedId === id} onSelect={onSelect} onDragStart={onDragStart}/></div>)}</div>
...
function VisualLayoutEditor({ config, mode, previewState, onChange }) {
  ...
  const move = (widgetId, destination) => { onChange(moveDisplayWidget(config, mode, widgetId, destination)); setSelectedId(widgetId); };
  ...
  return <div className="grid gap-3">
    ...
    {selected ? <div className="rounded-xl border border-amber-200 bg-amber-50 p-3"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-wide text-amber-800">Bloque seleccionado</p><p className="text-sm font-black">{DISPLAY_WIDGET_CATALOG[selected.type]?.label || selected.type}</p></div><div className="flex flex-wrap gap-1.5"><button type="button" onClick={() => shift(-1)} disabled={selectedIndex <= 0} className="inline-flex min-h-9 items-center gap-1 rounded-lg border bg-white px-2.5 text-[10px] font-bold disabled:opacity-30"><ChevronUp size={13}/>Antes</button><button type="button" onClick={() => shift(1)} disabled={!location || selectedIndex >= config.layouts[mode][location].length - 1} className="inline-flex min-h-9 items-center gap-1 rounded-lg border bg-white px-2.5 text-[10px] font-bold disabled:opacity-30"><ChevronDown size={13}/>Después</button><button type="button" onClick={() => remove(selectedId)} className="inline-flex min-h-9 items-center gap-1 rounded-lg border border-red-200 bg-white px-2.5 text-[10px] font-bold text-red-700"><X size={13}/>Quitar</button></div></div><div className="mt-3 flex flex-wrap gap-1.5"><span className="self-center text-[10px] font-bold text-gray-600">Mover a:</span>{allowedZones.map((target) => <button type="button" key={target} onClick={() => move(selectedId, target)} className={`rounded-lg border px-2.5 py-1.5 text-[10px] font-bold ${location === target ? "border-emerald-700 bg-emerald-700 text-white" : "bg-white"}`}>{zoneNames[target]}</button>)}</div></div> : <p className="rounded-xl border border-dashed bg-gray-50 px-3 py-2 text-center text-[10px] text-gray-500">Tocá cualquier bloque de la vista previa para moverlo o quitarlo.</p>}

    <div className="grid gap-2 rounded-xl border bg-white p-3 sm:grid-cols-[140px_minmax(0,1fr)]"><select value={addZone} onChange={(event) => setAddZone(event.target.value)} className="min-h-10 min-w-0 rounded-lg border bg-white px-2 text-xs">{allowedZones.map((target) => <option key={target} value={target}>{zoneNames[target]}</option>)}</select><select value="" onChange={(event) => { if (event.target.value) move(event.target.value, addZone); }} className="min-h-10 min-w-0 rounded-lg border bg-white px-2 text-xs"><option value="">+ Agregar un bloque a la pantalla…</option>{available.map((item) => <option key={item.id} value={item.id}>{DISPLAY_WIDGET_CATALOG[item.type]?.label || item.type}</option>)}</select></div>
