...
import {
  DISPLAY_MODES, DISPLAY_PRESETS, DISPLAY_SCHEDULE_DAYS, DISPLAY_WIDGET_CATALOG, DISPLAY_WIDGET_SIZES,
  DISPLAY_ZONES, SOCIAL_PLATFORMS, applyDisplayPreset, moveDisplayWidget, normalizeDisplayConfig,
...
function VisualZone({ zone, ids, config, state, selectedId, onSelect, draggingId, onDragStart, onDrop, horizontal = false }) {
  return <div onClick={() => onSelect("")} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); if (draggingId) onDrop(draggingId, zone); }} className={`relative min-h-0 min-w-0 rounded-md border border-dashed border-white/15 bg-black/5 p-1 ${horizontal ? "flex-none" : "h-full"}`}>
    ...
    <div className={`${horizontal ? "flex min-w-0 items-stretch gap-1 pt-1" : "flex h-full max-h-full min-h-0 flex-col gap-1 overflow-hidden pt-1"}`}>{ids.map((id, index) => { const widget = config.widgets[id]; const size = DISPLAY_WIDGET_SIZES[widget?.size] || DISPLAY_WIDGET_SIZES.medium; const previewScale = 0.9 + ((size.scale - 0.84) * 0.5); return <div key={id} onDragOver={(event) => { event.preventDefault(); event.stopPropagation(); }} onDrop={(event) => { event.preventDefault(); event.stopPropagation(); if (draggingId) onDrop(draggingId, zone, index); }} className="min-h-0 min-w-0 overflow-hidden" style={{ flexGrow: size.weight, flexShrink: 1, flexBasis: 0, minHeight: horizontal ? `${12 + size.weight * 4}px` : undefined }}><div className="h-full origin-top-left" style={{ width: `${100 / previewScale}%`, height: `${100 / previewScale}%`, transform: `scale(${previewScale})` }}><PreviewBlock id={id} widget={widget} state={state} selected={selectedId === id} onSelect={onSelect} onDragStart={onDragStart} onDragEnd={() => onDragStart("")}/></div></div>; })}</div>
...
function VisualLayoutEditor({ config, mode, previewState, onChange }) {
  ...
  const resize = (size) => {
    ...
    next.widgets[selectedId] = { ...next.widgets[selectedId], size };
