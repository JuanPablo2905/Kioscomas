...
export function CustomerDisplaySettings({ preferences, onChange, account, businessId, promotions = [], products = [], canConnectPayments = true }) {
  ...
  const mercadoPagoSection = (summaryLabel) => <details className="rounded-xl border bg-white p-3"><summary className="cursor-pointer text-sm font-black">{summaryLabel}</summary><div className="mt-4 grid gap-4">
  ...
  return <section className="rounded-2xl border border-emerald-200 bg-emerald-50/40 p-4 sm:p-5">
    ...
    {preferences.customerDisplayEnabled && <div className="mt-5 grid gap-5">
      ...
      {mercadoPagoSection("3. Mercado Pago: forma de cobro y QR")}
    ...
    {!preferences.customerDisplayEnabled && <div className="mt-5">{mercadoPagoSection("Mercado Pago: forma de cobro y QR")}<p className="mt-2 text-xs leading-5 text-gray-500">Podés dejar apagada la segunda pantalla y seguir usando Mercado Pago desde la caja o el celular.</p></div>}
  </section>;
}
