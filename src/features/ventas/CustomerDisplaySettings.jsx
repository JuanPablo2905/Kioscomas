...
export function CustomerDisplaySettings({ preferences, onChange, account, businessId, promotions = [], products = [] }) {
  ...
  return <section className="rounded-2xl border border-emerald-200 bg-emerald-50/40 p-4 sm:p-5">
    ...
    {preferences.customerDisplayEnabled && <div className="mt-5 grid gap-5">
      ...
      <details className="rounded-xl border bg-white p-3"><summary className="cursor-pointer text-sm font-black">2. Diseño, bloques y redes</summary><div className="mt-4"><CustomerDisplayLayoutEditor value={displayConfig} products={products} slideSeconds={Math.max(4, Number(preferences.customerDisplaySlideSeconds || 8))} thanksSeconds={Math.max(2, Number(preferences.customerDisplayThanksSeconds || 6))} onChange={(customerDisplayConfig) => set({ customerDisplayConfig })} onSlideSeconds={(customerDisplaySlideSeconds) => set({ customerDisplaySlideSeconds })} onThanksSeconds={(customerDisplayThanksSeconds) => set({ customerDisplayThanksSeconds })}/></div></details>
      ...
      <details className="rounded-xl border bg-white p-3"><summary className="cursor-pointer text-sm font-black">4. TVs y pantallas remotas</summary><div className="mt-4"><RemoteDisplayManager businessId={businessId} content={basePreviewState()}/></div></details>
