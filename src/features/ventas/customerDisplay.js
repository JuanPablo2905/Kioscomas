...
export async function openCustomerDisplay({ businessId, displayId = "", fullscreen = true, state = null }) {
  ...
  if (state) record && (record.state = state);
  if (window.kioscoDesktop?.customerDisplay?.open) {
    const result = await window.kioscoDesktop.customerDisplay.open({ channelId, displayId, fullscreen, state });
    if (state) publishCustomerDisplay({ businessId, state });
  ...
  opened.focus();
  window.setTimeout(() => state && publishCustomerDisplay({ businessId, state }), 350);
...
export function publishCustomerDisplay({ businessId, state }) {
  ...
  if (record) {
    record.state = state;
    record.channel.postMessage({ type: "state", state });
  }
  window.kioscoDesktop?.customerDisplay?.publish?.({ channelId, state }).catch?.(() => {});
}
