...
function PromotionMini({ promotion }) {
  return <article className="flex min-w-0 flex-1 items-center gap-[clamp(.65rem,1.4vmin,1.25rem)] rounded-[clamp(.9rem,2vmin,1.75rem)] bg-white/10 px-[clamp(.8rem,1.8vw,1.75rem)] py-[clamp(.65rem,1.35vmin,1.2rem)] text-white"><span className="shrink-0 rounded-full bg-amber-300 px-[clamp(.55rem,1vw,1rem)] py-[clamp(.25rem,.6vmin,.55rem)] text-[clamp(.6rem,1.05vmin,.9rem)] font-black uppercase text-amber-950">{promotion.badge || "PROMO"}</span><div className="min-w-0"><p className="truncate text-[clamp(.95rem,2vmin,1.65rem)] font-bold leading-tight">{promotion.title}</p>{promotion.description && <p className="truncate text-[clamp(.68rem,1.3vmin,1.1rem)] text-emerald-100">{promotion.description}</p>}</div></article>;
}

function PromotionsWidget({ state }) {
  const promotions = state.promotions || [];
  const rotating = usePromotion(promotions, state.slideSeconds, state.rotation);
  if (!promotions.length) return <div className="rounded-2xl border border-dashed border-white/25 px-4 py-[clamp(.7rem,1.3vmin,1.2rem)] text-center text-[clamp(.7rem,1.3vmin,1.1rem)] text-white/60">Las promociones activas aparecerán acá</div>;
...
function Idle({ state, config }) {
  return <div className="flex min-h-0 flex-1 flex-col overflow-hidden"><Zone state={state} config={config} mode="idle" zone="top" horizontal/><div className="grid min-h-0 flex-1 grid-cols-[minmax(0,23%)_minmax(0,1fr)_minmax(0,23%)] gap-[clamp(.25rem,.8vmin,.75rem)] overflow-hidden px-[clamp(.25rem,.8vw,.75rem)]"><aside className="min-h-0"><Zone state={state} config={config} mode="idle" zone="left"/></aside><section className="min-h-0"><Zone state={state} config={config} mode="idle" zone="center"/></section><aside className="min-h-0"><Zone state={state} config={config} mode="idle" zone="right"/></aside></div><Zone state={state} config={config} mode="idle" zone="bottom" horizontal/></div>;
}
...
function SaleCore({ state }) {
  ...
  return <div className="grid min-h-0 flex-1 gap-[clamp(.75rem,1.5vmin,1.5rem)] overflow-hidden px-[clamp(.75rem,1.5vw,1.75rem)] py-[clamp(.4rem,.9vmin,.8rem)] lg:grid-cols-[minmax(0,1fr)_minmax(300px,36%)]">
    <section className="flex min-h-0 flex-col overflow-hidden rounded-[clamp(1.25rem,3vmin,2.5rem)] bg-white text-gray-900 shadow-2xl"><div className="flex-none border-b px-[clamp(1.25rem,2.5vw,2.75rem)] py-[clamp(.7rem,1.5vmin,1.3rem)]"><h1 className="text-[clamp(1.35rem,2.8vmin,2.5rem)] font-bold">Tu compra</h1><p className="text-[clamp(.7rem,1.25vmin,1.05rem)] text-gray-500">{state.items.length} producto{state.items.length === 1 ? "" : "s"}</p></div><div ref={listRef} className="min-h-0 flex-1 divide-y overflow-y-auto overscroll-contain">{state.items.map((item, index) => { const hasPromotion = Number(item.promotion?.discount) > 0; return <div key={`${item.id}-${index}`} className={`grid gap-1 px-[clamp(1.25rem,2.5vw,2.75rem)] py-[clamp(.65rem,1.3vmin,1.15rem)] sm:grid-cols-[minmax(0,1fr)_auto] ${index === state.items.length - 1 ? "bg-amber-50" : ""}`}><div className="min-w-0"><p className="truncate text-[clamp(1rem,2vmin,1.65rem)] font-semibold">{item.name}</p><p className="text-[clamp(.7rem,1.3vmin,1.1rem)] text-gray-500">{item.quantityLabel || item.quantity}{state.showUnitPrices !== false ? ` × ${money(item.unitPrice)}` : ""}</p>{hasPromotion && <div className="mt-1 flex flex-wrap items-center gap-[clamp(.25rem,.6vmin,.5rem)]"><span className="rounded-full bg-violet-100 px-[clamp(.45rem,.8vw,.8rem)] py-0.5 text-[clamp(.6rem,1.05vmin,.9rem)] font-black uppercase text-violet-800">{item.promotion.badge || "PROMO"}</span><span className="max-w-[clamp(12rem,28vw,30rem)] truncate text-[clamp(.65rem,1.15vmin,1rem)] font-semibold text-violet-700">{item.promotion.name}</span><span className="text-[clamp(.65rem,1.15vmin,1rem)] font-bold text-emerald-700">Ahorrás {money(item.promotion.discount)}</span></div>}</div><div className="text-left sm:text-right">{hasPromotion && <p className="text-[clamp(.7rem,1.2vmin,1rem)] text-gray-400 line-through">{money(item.subtotal)}</p>}<p className="text-[clamp(1.1rem,2.3vmin,2rem)] font-bold">{money(hasPromotion ? item.finalSubtotal : item.subtotal)}</p></div></div>; })}</div></section>
    <aside className="flex min-h-0 flex-col overflow-hidden rounded-[clamp(1.25rem,3vmin,2.5rem)] bg-[#F6F1E7] p-[clamp(.9rem,2vmin,1.75rem)] text-[#173F3A] shadow-2xl"><div className="flex-none space-y-1 text-[clamp(.8rem,1.5vmin,1.25rem)]">{Number(state.discount || 0) > 0 && <><div className="flex justify-between gap-3"><span>Subtotal</span><b>{money(state.subtotal)}</b></div>{discountLines.map((line, index) => <div key={`${line.kind}-${line.label}-${index}`} className={`flex justify-between gap-3 ${line.kind === "manual" ? "text-blue-700" : "text-emerald-700"}`}><span className="min-w-0 truncate">{line.kind === "manual" ? "Descuento manual" : line.label}</span><b className="shrink-0">-{money(line.amount)}</b></div>)}</>}</div><div className="flex-none border-t border-[#173F3A]/20 pt-[clamp(.6rem,1.2vmin,1rem)]"><p className="text-[clamp(.7rem,1.3vmin,1.1rem)] font-bold uppercase tracking-[.16em] text-[#B95125]">Total a pagar</p><p className="truncate font-serif text-[clamp(2.5rem,6vmin,6rem)] font-black leading-tight">{money(state.total)}</p>{Number(state.discount || 0) > 0 && <p className="rounded-lg bg-emerald-100 px-2 py-1 text-center text-[clamp(.7rem,1.3vmin,1.1rem)] font-black text-emerald-800">Ahorraste {money(state.discount)}</p>}</div>{payment && <div className="mt-[clamp(.4rem,1vmin,.9rem)] min-h-0 overflow-y-auto rounded-2xl bg-white p-[clamp(.65rem,1.5vmin,1.25rem)] text-center"><p className="text-[clamp(.65rem,1.15vmin,1rem)] font-semibold text-gray-500">Medio elegido</p><p className="text-[clamp(1.25rem,2.8vmin,2.5rem)] font-bold">{payment.method}</p>{payment.method === "Efectivo" && state.showChange !== false && <div className="mt-1 grid grid-cols-2 gap-2 border-t pt-1 text-left"><div><span className="text-[clamp(.65rem,1.1vmin,.95rem)] text-gray-500">Recibido</span><b className="block text-[clamp(.9rem,1.7vmin,1.4rem)]">{money(payment.received || 0)}</b></div><div><span className="text-[clamp(.65rem,1.1vmin,.95rem)] text-gray-500">Vuelto</span><b className="block text-[clamp(.9rem,1.7vmin,1.4rem)] text-emerald-700">{money(Math.max(0, payment.change || 0))}</b></div></div>}{showQr && <div className="mt-1"><img src={state.paymentQrImage} alt="Código QR para pagar" className="mx-auto aspect-square max-h-[min(26vh,15rem)] max-w-full rounded-xl border object-contain p-1"/><p className="text-[clamp(.65rem,1.1vmin,.95rem)] font-semibold text-gray-500">Escaneá el QR y mostrá el comprobante.</p></div>}</div>}<p className="mt-auto flex-none pt-1 text-center text-[clamp(.6rem,1vmin,.9rem)] text-[#173F3A]/60">El pago se confirma en la caja.</p></aside>
  </div>;
}
