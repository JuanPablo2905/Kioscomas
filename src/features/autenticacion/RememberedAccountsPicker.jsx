...
export function RememberedAccountsPicker({ apiUrl, deviceId, onDeviceCredentialLogin, onUseAnotherAccount }) {
  ...
  const tryBiometric = async (entry) => {
    ...
    setWorking(true);
    try {
      const { deviceCredential, unlockMaterialBytes } = await unlockAccountWithBiometric(entry.id, deviceId);
  ...
  return (
    <div className="login-screen flex min-h-screen w-full items-center justify-start bg-gray-50 sm:justify-center">
      <div className="login-card w-full max-w-sm min-w-0 rounded-xl border border-gray-200 bg-white p-4 sm:p-6">
        ...
        <p className="mb-5 text-sm text-gray-500">{selected ? "Ingresá tu PIN para entrar." : "¿Quién va a usar Kiosco+ ahora?"}</p>
        ...
        {!selected ? <div className="space-y-2">
          {accounts.map((account) => (
            <button key={account.id} type="button" onClick={() => setSelected(account)} className="flex min-h-14 w-full items-center gap-3 rounded-xl border border-gray-200 px-3 py-2 text-left hover:bg-gray-50">
              ...
              {account.hasBiometric && <Fingerprint size={18} className="shrink-0 text-gray-400"/>}
            </button>
          ))}
          ...
          <button type="submit" disabled={working || pin.length < 4} className="brand-cta mb-2 flex min-h-11 w-full items-center justify-center gap-2 rounded-lg bg-gray-900 px-4 py-2.5 text-sm font-semibold text-white disabled:cursor-wait disabled:opacity-60"><LogIn size={16}/>{working ? "Entrando..." : "Entrar"}</button>
