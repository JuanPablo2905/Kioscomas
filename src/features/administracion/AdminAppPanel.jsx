...
import { Archive, ArrowLeft, Barcode, Bug, CalendarDays, CheckCircle2, Cloud, Clock, CreditCard, Gift, History, KeyRound, Lightbulb, MonitorUp, Pencil, Plus, Search, Settings2, Shield, Store, Trash2, UserRound, UsersRound, X, XCircle } from "lucide-react";
...
export function AdminAppPanel({ cuentas, setCuentas, datos, setDatos, notas, setNotas, reportes = [], setReportes, onOpenNegocio, onOpenNegocioSecondary, onLogout, onOpenSettings, syncStatus, onSyncNow }) {
  ...
  const activationRequest = async (path, options = {}) => {
    const config = loadCloudConfig();
    const session = cloudSession(config.apiUrl);
    if (!config.apiUrl || !session?.user?.businessId) throw new Error("Primero conectá la cuenta administradora con la nube desde Configurar.");
    ...
    if (!response.ok) throw new Error(detail.error || "No se pudo administrar las claves de instalación.");
    return detail;
  };
  ...
  const loadActivations = async () => {
    ...
    setActivationError("");
    try {
      const detail = await activationRequest("/v1/admin/activation-codes");
      setActivationCodes(detail.codes || []);
      setActivatedDevices(detail.activations || []);
    } catch (error) {
      setActivationError(error?.message || "No se pudieron cargar las claves.");
    } finally {
  ...
  useEffect(() => {
    loadActivations();
    const reload = () => loadActivations();
    window.addEventListener("kiosco-cloud-session-changed", reload);
  ...
  const createActivationCode = async () => {
    ...
    try {
      ...
      setGeneratedCode(detail);
      setActivationCodes((previous) => [detail.item, ...previous.filter((item) => item.id !== detail.item.id)]);
      setActivationForm((previous) => ({ ...previous, label: "" }));
    } catch (error) {
      setActivationError(error?.message || "No se pudo generar la clave.");
    } finally {
      setActivationLoading(false);
    }
  };

  const revokeActivationCode = async (id) => {
    setActivationError("");
    try {
      const detail = await activationRequest(`/v1/admin/activation-codes/${encodeURIComponent(id)}/revoke`, { method: "POST" });
      setActivationCodes((previous) => previous.map((item) => item.id === id ? detail.item : item));
    } catch (error) { setActivationError(error?.message || "No se pudo desactivar la clave."); }
  };

  const revokeActivatedDevice = async (deviceId) => {
    setActivationError("");
    try {
  ...
  return (
    <div className="kiosco-themed admin-app-shell min-h-screen bg-gray-50 p-5 text-gray-900 md:p-8">
      <ConfirmDialog open={Boolean(cuentaABorrarId)} title="Eliminar cuenta" message={`Se borrará la cuenta de ${negocios.find((item) => item.id === cuentaABorrarId)?.nombreNegocio || "este negocio"} y todos sus datos. Esta acción no se puede deshacer.`} confirmLabel="Eliminar definitivamente" danger onCancel={() => setCuentaABorrarId(null)} onConfirm={confirmarBorrado}/>
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mb-7 rounded-2xl border border-gray-200 bg-white p-4 sm:p-5">
          ...
          {activationError && <p className="mt-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-700">{activationError}</p>}

          <div className="mt-5 grid gap-4 lg:grid-cols-2">
            <div className="overflow-hidden rounded-xl border border-gray-200">
              <div className="flex items-center justify-between bg-gray-50 px-4 py-3"><h3 className="text-sm font-bold">Claves generadas</h3><span className="text-xs text-gray-500">{activationCodes.length}</span></div>
              <div className="max-h-72 divide-y overflow-auto">
                {!activationLoading && activationCodes.length === 0 && <p className="px-4 py-6 text-center text-sm text-gray-400">Todavía no generaste claves.</p>}
                ...
                {editandoId === cuenta.id ? (
                  ...
                  <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
                    ...
                    <div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:flex-wrap">
                      ...
                      <button onClick={() => onOpenNegocioSecondary?.(cuenta.id)} disabled={!canAccessAccount(cuenta)} className="flex items-center justify-center gap-1.5 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-800 disabled:opacity-30"><MonitorUp size={14}/>Otra pantalla</button>
                ...
                {usuariosCuentaId === cuenta.id && <div className="mt-4 overflow-hidden rounded-xl border border-gray-200 bg-gray-50/60">
                  ...
                  <div className="divide-y">
                    {[{ ...cuenta, rol: "Dueño", tipoCuenta: "dueno" }, ...(cuenta.empleados || []).map((empleado) => ({ ...empleado, tipoCuenta: "empleado" }))].map((sujeto) => <div key={`${sujeto.tipoCuenta}-${sujeto.id}`} className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                      <div className="flex min-w-0 items-center gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-white text-[#1C4A44] shadow-sm"><UserRound size={17}/></span><div className="min-w-0"><p className="flex min-w-0 items-center gap-1.5 text-sm font-semibold"><span className="min-w-0 truncate">{sujeto.nombre || "Sin nombre"}</span><span className="shrink-0 rounded-full bg-white px-2 py-0.5 text-[10px] font-medium text-gray-600">{sujeto.rol || "Empleado"}</span></p><p className="truncate text-xs text-gray-500">Usuario: @{sujeto.usuario} · {sujeto.email || "sin correo"}</p></div></div>
