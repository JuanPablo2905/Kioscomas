...
export function AdminNotificationCenter({ accounts = [] }) {
  ...
  return <section className="mb-7 rounded-2xl border border-gray-200 bg-white p-4 sm:p-5">
    ...
    <div className="admin-notification-tabs mt-5 grid grid-cols-3 gap-2 rounded-xl p-1"><button type="button" onClick={() => setTab("admin")} className={`admin-notification-tab flex min-h-11 items-center justify-center gap-2 rounded-lg px-2 text-xs font-bold ${tab === "admin" ? "is-active shadow-sm" : ""}`}><Bell size={15}/>Bandeja administrativa {adminItems.length > 0 && <span className="rounded-full bg-red-600 px-1.5 py-0.5 text-[9px] text-white">{adminItems.length}</span>}</button><button type="button" onClick={() => setTab("send")} className={`admin-notification-tab flex min-h-11 items-center justify-center gap-2 rounded-lg px-2 text-xs font-bold ${tab === "send" ? "is-active shadow-sm" : ""}`}><Send size={15}/>Enviar aviso</button><button type="button" onClick={() => setTab("history")} className={`admin-notification-tab flex min-h-11 items-center justify-center gap-2 rounded-lg px-2 text-xs font-bold ${tab === "history" ? "is-active shadow-sm" : ""}`}><History size={15}/>Historial</button></div>
