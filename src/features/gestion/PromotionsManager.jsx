...
export function PromotionsManager({ products = [], promociones = [], setPromociones }) {
  ...
  return <div className="grid min-w-0 gap-5 xl:grid-cols-[440px_1fr]">
    <div className="min-w-0 rounded-xl border bg-white p-4 sm:p-5">
      <div className="flex min-w-0 items-center gap-2"><BadgePercent size={18} className="shrink-0"/><h3 className="min-w-0 break-words font-semibold">{editingId != null ? "Editar promoción" : "Nueva promoción o combo"}</h3></div>
      ...
      <div className="mt-4 grid gap-3">
        ...
        <div className={editingId != null ? "grid grid-cols-2 gap-2" : ""}>{editingId != null && <button type="button" onClick={reset} className="flex min-h-11 items-center justify-center gap-2 rounded-lg border px-3 py-2 text-sm font-semibold"><X size={16}/>Cancelar</button>}<button disabled={!valid} onClick={save} className="flex min-h-11 w-full items-center justify-center gap-2 rounded-lg bg-gray-900 px-3 py-2 text-sm font-bold text-white disabled:opacity-40">{editingId != null ? <Pencil size={16}/> : <Plus size={16}/>} {editingId != null ? "Guardar cambios" : "Crear promoción"}</button></div>
      </div>
    </div>
