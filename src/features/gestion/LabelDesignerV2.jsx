...
export function LabelDesignerV2({ products, templates = [], setTemplates }) {
  ...
  return <div className="min-w-0 rounded-xl border bg-white p-3 sm:p-4">
    <div className="flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between"><div className="min-w-0"><h3 className="font-semibold">Editor libre de etiquetas</h3><p className="text-sm text-gray-500">Arrastrá el nombre, precio, código y demás elementos directamente sobre la etiqueta.</p></div><button disabled={!chosen.length} onClick={() => window.print()} className="flex min-h-10 w-full items-center justify-center gap-2 rounded-lg bg-gray-900 px-3 py-2 text-sm text-white disabled:opacity-40 sm:w-auto"><Printer size={16}/>{chosen.length ? `Imprimir ${chosen.length * (Number(config.copies) || 1)} etiqueta(s)` : "Imprimir"}</button></div>
