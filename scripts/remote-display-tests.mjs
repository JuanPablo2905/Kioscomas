...
try {
  ...
  const paymentPresentation = await request("/v1/payments/presentations", { method: "POST", headers, body: JSON.stringify({ target: "mobile", mode: "static_qr", method: "Pago combinado", amount: 13000, payments: [{ metodo: "Efectivo", monto: 3000 }, { metodo: "Mercado Pago", monto: 10000 }], qrImage: "data:image/png;base64,iVBORw0KGgo=" }) });
  ...
  assert(closedPayment.response.ok && (await request("/v1/payments/presentations/active", { headers })).value.presentations?.length === 0, "cerrar el cobro lo retira de los demás celulares");
  const created = await request("/v1/business-displays", { method: "POST", headers, body: JSON.stringify({ name: "TV vidriera", content: { businessName: "Comercio", secret: "NO-DEBE-SALIR", costs: [900], config: { operationMode: "sale-and-ads", placements: { idle: { hours: { x: 4, y: 12, width: 42, height: 55, z: 3 } } }, widgets: { hours: { schedule: [{ day: "monday", enabled: true, open: "08:00", close: "20:00" }] } } }, promotions: [{ id: "promo", title: "2x1", internalCost: 500 }] } }) });
  ...
  assert(created.value.display?.content?.config?.placements?.idle?.hours?.width === 42 && created.value.display?.content?.config?.placements?.idle?.hours?.height === 55 && created.value.display?.content?.config?.widgets?.hours?.schedule?.find((entry) => entry.day === "monday")?.open === "08:00", "la pantalla remota conserva posición, ancho, alto y horarios configurados");
