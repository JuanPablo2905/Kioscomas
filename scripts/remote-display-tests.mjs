...
try {
  ...
  assert(created.value.display?.content?.config?.widgets?.hours?.sizePercent === 147 && created.value.display?.content?.config?.widgets?.hours?.schedule?.find((entry) => entry.day === "monday")?.open === "08:00", "la pantalla remota conserva tamaños manuales y horarios configurados");
