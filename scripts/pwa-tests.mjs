...
const checks = [
  ...
  [customerDisplay.includes("data-text-density") && customerDisplay.includes("customer-social-card__qr") && customerDisplayLayout.includes("customer-social-card__text") && styles.includes('data-text-density="long"') && styles.includes("@container (min-aspect-ratio: 7 / 4)") && styles.includes("flex-direction: row"), "Las tarjetas de redes deben reducir el texto y mover el QR al costado cuando sean anchas y bajas."],
