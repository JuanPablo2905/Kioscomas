...
export function createEmailService({ apiKey, from, replyTo = "", appUrl, fetchImpl = globalThis.fetch, testMode = false } = {}) {
  ...
  return {
    ...
    sendEmailVerification: ({ to, name, businessName, verifyUrl, expiresInHours, accountId }) => send({
      to,
      subject: "Confirmá tu correo para entrar a Kiosco+",
      preview: "Un paso más: confirmá tu correo para poder iniciar sesión.",
      category: "Nueva cuenta",
      title: "¡Bienvenido a Kiosco+!",
      greeting: `Hola ${name || ""}`.trim(),
      paragraphs: [`Creamos la cuenta de ${businessName || "tu negocio"}. Confirmá tu correo para poder iniciar sesión.`, "En cuanto lo confirmes arrancás directo con 30 días de prueba, sin cargo ni tarjeta."],
      actionLabel: "Confirmar mi correo",
      actionUrl: verifyUrl,
      footnote: `Este enlace vence en ${expiresInHours} horas. Si no creaste esta cuenta, ignorá este correo.`,
      idempotencyKey: `verify-email/${accountId}`,
    }),
    sendNewAccountAdminAlert: ({ to, businessName, ownerName, ownerEmail, accountId }) => send({
      to,
      subject: `Cuenta nueva: ${businessName || "sin nombre"}`,
      preview: "Se registró una cuenta nueva en Kiosco+.",
      category: "Panel de administración",
      title: "Se creó una cuenta nueva",
      greeting: "Hola",
      paragraphs: [
        `${businessName || "Un negocio"} acaba de crear su cuenta. Dueño: ${ownerName || "sin nombre"} (${ownerEmail || "sin correo"}).`,
        "Ya arrancó su beta de 30 días sin que haga falta aprobarla. Entrá al panel si querés revisarla, bloquearla o darle más días de prueba.",
      ],
      actionLabel: "Abrir el panel de administración",
      actionUrl: configuredAppUrl,
      footnote: `ID de la cuenta: ${accountId}`,
      idempotencyKey: `new-account-alert/${accountId}`,
    }),
    sendAccountReady: ({ to, name, businessName, accountId, eventId }) => send({
      ...
      actionUrl: configuredAppUrl,
      footnote: "Si ingresás desde un dispositivo nuevo, sólo necesitás tu usuario y contraseña.",
